...
class GroupModerationApiController extends Controller
{
    ...
    public function suspend(Request $request, Group $group, User $user)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "{$user->name} has been suspended from this group.",
        ]);
    }
    ...
    public function block(Request $request, Group $group, User $user)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "{$user->name} has been blocked from this group.",
        ]);
    }
    ...
    public function reinstate(Group $group, User $user)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "{$user->name} has been reinstated in this group.",
        ]);
    }
    ...
    private function authorizeModeration(Group $group, User $user): void
    {
        abort_unless(Auth::user()->canManageGroup($group), 403);
        abort_unless($group->isMember($user->id), 404);
        abort_unless((int) $user->id !== (int) Auth::id(), 422);
